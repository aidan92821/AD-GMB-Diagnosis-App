...
def simulate(run_id: int, abundance: dict, user_diet: dict[str, float], runner: QiimeRunner, 
             antibiotic: str=None, n_steps: int=100, dt: float=0.1) -> dict:
    '''
    ...
    for genus, model_path in models.items():
        ...
        for step in range(n_steps):
            with model:
                ...
                for metabolite, flux in nutrients.items():
                    rxn_id = "EX_" + metabolite.replace("[e]", "(e)")
                    # (Michaelis-Menten) limit uptake by availability
                    if rxn_id not in model.reactions:  # add this check
                        continue
                    rxn = model.reactions.get_by_id(rxn_id)
                    rxn.lower_bound = max(-flux / dt, rxn.lower_bound)
                ...
                for met_id in nutrients:
                    rxn_id = "EX_" + met_id.replace("[e]", "(e)")
                    if rxn_id in model.reactions:
                        flux = solution.fluxes[rxn_id]
                        nutrients[met_id] = max(
                            nutrients[met_id] + flux * biomass * dt, 0
                        )
    ...
    growth_rates = {genus: record[-1]["growth_rate"] for genus, record in history.items()}
    scfa_production = {genus: record[-1]["scfa_production"] for genus, record in history.items()}

    # calculate the new abundances for each simulated genus
    new_abundance = calc_new_abundance(abundance, growth_rates)
...
def calc_new_abundance(original_abundance: dict, growth_rates: dict) -> dict:
    '''
    ...
    sim_prop = sum(original_abundance[genus] for genus in growth_rates)

    # use each genus' growth rate to recalculate its abundance by scaling its original
    scales = {}
    for genus, rate in growth_rates.items():
...
def plot_sim_results(results: dict, original_abundance: dict) -> dict:
    ...
    # STATIC PLOTS
    # growth rates by genus
    fig1, ax1 = plt.subplots()
    ax1.barh(list(results['growth_rates'].keys()), list(results['growth_rates'].values()))
    ax1.set_xlabel("Predicted Growth Rate (1/hr)")
    ax1.set_title("Genus Growth Rates Under Modified Diet")
    path1 = os.path.join(tmp_dir, "growth_rates.png") # TEMP
    fig1.savefig(path1, bbox_inches='tight') # TEMP
    plt.close(fig1)
    plots['growth_rates'] = fig1
    ...
    wedges0, _, _ = axes[0].pie(top10_old.values(), labels=top10_old.keys(), autopct="%1.1f%%",
                                              colors=[genus_color_map[g] for g in top10_old.keys()],
                                              pctdistance=0.85, labeldistance=None)
    # axes[0].legend(wedges0, top10_old.keys(), loc="lower left", fontsize=7, bbox_to_anchor=(-0.3, 0))

    axes[0].set_title("Before Diet Change")
    wedges1, _, _ = axes[1].pie(top10_new.values(), labels=top10_new.keys(), autopct="%1.1f%%",
                                              colors=[genus_color_map[g] for g in top10_new.keys()],
                                              pctdistance=0.85, labeldistance=None)
    axes[1].set_title("After Diet Change (Predicted)")
    # axes[1].legend(wedges1, top10_new.keys(), loc="lower right", fontsize=7, bbox_to_anchor=(1.3, 0))
    ...
    fig2.legend(
        ...
        loc="lower center",
        ncol=3,              # adjust columns based on how many genera
        fontsize=8,
        bbox_to_anchor=(0.5, -0.15)  # below the charts
    )
    ...
    scfa_df.plot(kind='bar', stacked=True, title="Predicted SCFA Production by Species", ax=ax3)
    ...
    for genus, df in dfs.items():
        ax4.plot(df["step"], df["biomass"], label=genus)
    ...
    key_nutrients = ["fru[e]", "glc_D[e]", "arab_L[e]", "ac[e]"]
    fig7, axes7 = plt.subplots(len(dfs), 1, figsize=(10, 4 * len(dfs)), sharex=True)
    if len(dfs) == 1:
        axes7 = [axes7]

    for ax, (genus, df) in zip(axes7, dfs.items()):
        for met in key_nutrients:
            col = f"conc_{met}"
            if col in df.columns:
                ax.plot(df["step"], df[col], label=met)
        ax.set_title(genus, fontsize=9)
        ax.set_ylabel("Concentration (mmol)")
        ax.legend(fontsize=7)

    axes7[-1].set_xlabel("Time Step")
    fig7.suptitle("Nutrient Depletion Over Time", fontsize=12)
    plt.tight_layout()
    path7 = os.path.join(tmp_dir, "dfba_nutrients.png")
    fig7.savefig(path7, bbox_inches='tight')
    plt.close(fig7)
    plots["dfba_nutrients"] = fig7
    # DYNAMIC PLOTS
...
def get_abundance_shift_stats(abundance_old, abundance_new) -> pd.DataFrame:
    ...
    results['pct_change'] = np.where(
        condition=(results['before'] != 0),
        x=(results['abs_change'] / results['before']) * 100,
        y=0
    )
